...
Hola, terrícola, te damos la bienvenida a la carpeta destinada para poner los materiales del taller llevado a cabo en el Tapancazo: Congreso Intergaláctico de Digitalización, el cual consta de dos partes:

1. *Digitalización de las obras elegidas*. Cada participante, en equipo o de manera individual, aprendierá digitalizar con la obra de su elección.

2. *Digitalización de los cartones de Gabriel Portillo del Toro*. Los participantes colaborarán en la digitalización de los cartones de [GPT](https://www.youtube.com/watch?v=Wh4o1HO5rPQ), lo cual forma parte de la primera fase de este proyecto.

La idea general del taller consiste en que ustedes nos ayuden en nuestro proyecto, a la par que nosotras les enseñamos a digitalizar.

## Proyecto Editorial Gabriel Portillo del Toro

GPT fue un terrícola muy querido por nosotras ): y por ese motivo decidimos digitalizar todos sus cartones (ilustraciones) relativos a varios temas políticos o sociales de su época. Como deseamos difundir su obra, de manera completamente libre y accesible para todas las personas, hemos pensado en un proyecto que comprende las siguientes fases:

1. *Digitalización*. El trabajo duro de pasar de lo impreso a lo digital es lo que se llevará a cabo en este taller. ¡Gracias por ayudarnos!

2. *Catalogación*. Para poder cotejar todo el material digitalizado se realizará una clasificación de los cartones: dejar todo bien acomodadito para que cualquiera pueda indagar en los archivos, incluyendo los datos necesarios para identificar cada elemento.

3. *Subida*. A la par de la catalogación o poco después (todavía no lo decidimos :D), todo el material catalogado estará disponible en otro repositorio de Perro Triste para que cualquier persona pueda usarlo sin problemas. Además, para quienes están poco acostumbrados al uso de `git` pero también desean acceder a todo el material, crearemos una plataforma de fácil acceso para el público general. (:

4. *Publicación digital*. Por último, como parte del segundo proyecto del Taller de Procesos Editoriales de este perrito, haremos una selección de los cartones y a partir de ahí editaremos y desarrollaremos una publicación digital abierta, libre y gratuita. Aunque probablemente tardemos un poco, ¿sale? ¡Ah!, si tienes alguna idea o sugerencia sobre cómo hacerla, mándanos un mensajito (contacto@perrotriste.org).

## Proceso de digitalización

A sabiendas que los terrícolas constantemente discuten por el nombre de las cosas, podemos decir que el proceso para obtener una publicación digital a partir de una obra impresa consta de dos fases:
...
En esta primera etapa se pasa del formato impreso a un facsímil cuyo archivo de salida es un PDF. Esto involucra las siguientes acciones:
  ...
  1. *Digitalizar*. Comprende el trabajo duro de utilizar un escáner para obtener imágenes en formato TIFF o PNG (emplear una resolución mínima de 300 dpi).
...
2. *Editar*. Desafortunadamente, durante la extracción se pierde el formato original del texto o este queda poco uniforme, además del inconveniente de que el reconocimiento de caracteres no es del todo exacto. Por estos motivos, es necesario editar la obra para una mayor legibilidad y deleite. ;)
...
* Scan Tailor
  ...
  * Instalación:
    ...
    * Mac OS X: `brew install homebrew/x11/scantailor` desde la terminal y con Homebrew (se abre desde la terminal con `scantailor`). Más adelante, en la sección de Mac OS X, se indica cómo instalar Homebrew.
...
* Ghostscript
  ...
  * [Uso](http://www.tjansson.dk/2012/04/compressing-pdfs-using-ghostscript-under-linux/):
    * GNU/Linux o Mac: `gs -sDEVICE=pdfwrite -dCompatibilityLevel=1.4 -dPDFSETTINGS=/ebook -dNOPAUSE -dQUIET -dBATCH -sOutputFile=output.pdf input.pdf`.
...
* Brackets
  ...
  * Instalación:
    * Windows, GNU/Linux y Mac OS X: [http://brackets.io/](http://brackets.io/).
...
A continuación se enumeran los tipos de *software* que no son compartidos por Windows, GNU/Linux y Mac OS X, pero que nos ayudarán en nuestra travesía.
...
* Tesseract
  * Para reconocer caracteres. No olvides instalar por lo menos el lenguaje español `spa`.
...
Sin embargo, sabemos que no a todos les es cómodo editar directamente en HTML. Si vas a utilizar un procesador de textos, no uses *software* privativo; no uses el feo Word (desaconsejado por los estándares UFO). Nosotras recomendamos LibreOffice, el cual está avalado por CULTUR.
...
En la práctica la carpeta de tu EPUB puede ser nombrada como prefieras, aunque lo recomendable es emplear el nombre de la obra para evitar confusiones.
...
Si esto no logra orientarte, no olvides en consultar nuestras [infografías](https://github.com/ColectivoPerroTriste/Tapancazo/tree/master/infograf%C3%ADas), especialmente la que trata sobre los [procesos de digitalización](https://github.com/ColectivoPerroTriste/Tapancazo/blob/master/infograf%C3%ADas/procesos-editoriales-digitalizaci%C3%B3n/esquema-digitalizacion.svg), donde puedes ver una relación entre los pasos a seguir y los programas para utilizar según tu tipo de nave espacial.

Ve toda esta documentación como un mapa y un manual, pero si aún persisten las dudas nuestro sombrero también es casco intergaláctico, por lo que con gusto somos tus mecánicos. Solo chíflanos a contacto@perrotriste.org.
